          ...
          <div className="p-6">
              ...
              <div className="space-y-2 mb-3">
                {cartItems.map(item => (
                  <div key={item.id} className="flex justify-between text-sm text-gray-700">
                    ...
                    <span>${(item.currentUnitPrice * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-gray-200 pt-2 font-semibold text-gray-900 flex justify-between">
                <span>Total (con IVA 21%):</span>
                <span>${(cartItems.reduce((total, item) => total + (item.currentUnitPrice * item.quantity), 0) * 1.21).toFixed(2)}</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="border-t border-gray-200 pt-6 flex gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white font-semibold py-2 rounded-lg transition disabled:cursor-not-allowed"
                  disabled={loading}
                >
